...
public class ConnectDB {
    ...
    private void createTables() {
        try {
            ...
            conn.createStatement().execute("Create Table pastSummary(summaryLocation varchar(255), Uname varchar(50), DocLoc varchar(255), keyphrase varchar(255), PRIMARY KEY (summaryLocation), FOREIGN KEY (Uname) REFERENCES Users(Username), FOREIGN KEY (DocLoc) REFERENCES inputDoc(DocLocation))");
    ...
    public void inputSummary(String SummaryLocation , String keyphrase, String uName, String DocLoc) {
        try {
            conn.createStatement().execute(String.format("Insert into pastSummary(SummaryLocation, keyphrase, Uname, DocLoc) Values('%s','%s','%s','%s')", SummaryLocation, keyphrase, uName, DocLoc));
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public String getData() {
        ...
        ResultSet r;
        try {
            r = conn.createStatement().executeQuery("Select * from Users u,inputDoc i,pastSummary p where u.Username = i.Uname and u.Username = p.Uname");
            while (r.next()) {
                data += r.getString(1) + " " + r.getString(2) + "\n";
            }
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public String checkSummaryKey(String DocLoc, String keyphrase) {
        ResultSet r;
        try {
            r = conn.createStatement().executeQuery(String.format("Select * From pastSummary Where keyphrase = '%s' and DocLoc = '%s'", keyphrase, DocLoc));
            if (r.next()) {
                return r.getString(1);
            } else {
                return null;
